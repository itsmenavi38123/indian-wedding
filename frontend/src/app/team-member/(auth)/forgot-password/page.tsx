...
import { teamMemberForgotPassword } from '@/services/api/teamMemberAuth';
...
const ForgotPasswordPage = () => {
  ...
  const form = useForm<ForgotPasswordInput>({
    ...
    defaultValues: { email: teamMemberLoginEmail || '' },
  });
  ...
  const {
    ...
    mutationFn: async (data: ForgotPasswordInput) => teamMemberForgotPassword(data),
    onSuccess: (data) => {
      dispatch(setEmail(emailValue));
      toast.success(data?.message ?? 'Password reset otp sent to your email.');
      router.push('/team-member/otp-verify');
    },
    onError: () => {
      // toast.error(
    ...
    <div className="flex-1 flex items-center justify-center bg-[#f5f5f5] px-[10px] md:px-[30px] pt-[120px] md:pt-[180px] pb-[80px] md:pb-[120px]">
      <div className="w-full max-w-md border rounded-2xl shadow-md p-6 px-[20px]  md:px-[30px] bg-card">
        ...
        <div className="text-center mt-4">
          <Link href="/team-member/login" className="text-sm text-primary hover:underline">
            Back to Login
          </Link>
        </div>
      </div>
