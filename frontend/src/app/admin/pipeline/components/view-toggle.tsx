...
export default function ViewToggle({ value, onChange }: Props) {
  const base =
    'px-2 sm:px-3 py-1.5 text-xs sm:text-sm rounded-md border transition-colors select-none focus:outline-none focus:ring-2 focus:ring-blue-500 flex-1 sm:flex-initial';
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-2 w-80 sm:w-auto">
      <button
        type="button"
        className={`${base} ${value === 'kanban' ? active : inactive}`}
