...
import { useGetAdminTeamById } from '@/services/api/admin';

const ViewVendorTeamPage = () => {
  const { id: teamId } = useParams<{ id: string }>();
  const { data, isLoading, error } = useGetAdminTeamById(teamId);
          ...
          {members.length > 0 ? (
            <div className="flex flex-wrap gap-4">
              {members.map((member: any, index: number) => (
                <div
                  key={index}
                  className="border w-fit min-w-xs flex flex-1 rounded-xl p-4 flex-col gap-2 bg-card hover:shadow-md transition-shadow"
                >
                  <div className="flex items-center gap-2">
                    <User className="h-4 w-4 text-muted-foreground" />
                    <p className="font-medium text-base text-gray-700">
                      {member.teamMember?.name || 'N/A'}
                    </p>
                  </div>
