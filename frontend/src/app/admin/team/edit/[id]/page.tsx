...
import { VendorFormValues } from '@/app/(components)/vendors/components/VendorForm';
import { UpdateTeamWithMembersPayload } from '@/services/api/vendors';
import { PageHeader } from '@/components/ui/pageHeader';
import AdminFormPage from '../../_components/AdminForm';
import { useGetAdminTeamById, useUpdateTeamWithMembers } from '@/services/api/admin';

const EditTeam = () => {
  const { id: teamId } = useParams<{ id: string }>();
  const router = useRouter();
  const { data, isLoading } = useGetAdminTeamById(teamId);
