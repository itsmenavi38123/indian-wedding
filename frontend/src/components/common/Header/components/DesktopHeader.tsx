  ...
  return (
    <header className="hidden h-[172px] md:flex w-full  pt-[27px] pb-[60px] fixed top-0 left-0 z-50 items-center justify-between px-[15px] md:px-[20px] lg:px-[20px] xl:px-[60px]  bg-[image:var(--primary-gradient)]">
      ...
      <nav className="flex items-center md:gap-[10px] lg:gap-[15px] xl:gap-8">
        {leftNav.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="text-sm font-medium hover:text-gold transition-colors text-white leading-1.5 text-[16px] md:text-[14px] lg:text-[16px] xl:text-[18px]"
          >
            {item.name}
          </Link>
        ))}
      ...
      <div className="flex items-center md:gap-[10px] lg:gap-[15px]  xl:gap-8">
        ...
        <Button
          size="sm"
          className="ml-2 h-[45px] flex justify-center items-center bg-gold text-center leading-1.5 text-[16px] md:text-[14px] lg:text-[16px] xl:text-[18px] font-normal tracking-[1px] hover:text-gold hover:bg-white transition-colors text-white cursor-pointer md:px-[16px] md:py-[8px]  lg:px-[22px]  lg:py-[10px] xl:px-[36px] xl:py-[12px]"
